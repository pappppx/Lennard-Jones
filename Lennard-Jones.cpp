...
#define L 4     //Lado caja
#define h 0.002 //Paso
#define t 60    //Tiempo total
#define pi 3.141592
...
int main()
{
    ...
    for ( i = 0; i < N; i++)
    {   /*do        //SI QUEREMOS QUE LA POSICION Y VELOCIDAD SEAN ALEATORIAS
        {
    ...
    }                   //SI QUEREMOS QUE V=0 Y POSICION ORDENADA Y DISTRIBUIDA HOMOGENEAMENTE
    x[1]=0.5;
    y[1]=0.5;
    x[2]=1.5;
    y[2]=0.5;
    ...
    xi=x[6];           //Para calcular la posicion inicial de una única partícula y luego ver sus fluctuaciones
    yi=y[6];
    contador=0;
    contador2=0;
    T=0;
    ...
    {
        //if(tx>=20)         //ESTO SE HA USADO CUANDO QUERÍAMOS REGISTRAR LA TEMPERATURA A PARTIR DE CIERTO TIEMPO
    //{
        aux=0;
    ...
    //}
    if (fmod(contador, 10)==0 || tx==0)     //cada 10 pasos por escribir menos en los ficheros y que todo se haga más rápido
    {
        //Energia
        ek=0.;
        ep=0.;
    ...
    ent << tx << "   "<< et << endl; 
        ...
        /*if (contador>5000)        //Para asegurar que el sistema está en equilibrio
        {
...
for (i=1;i<N;i++)
    {
       ...
       /* if (contador==5000)       //Esto se ha usado en la correlación de pares, para calentar el sistema y ver como cambiaba el histograma
        {
            for(i=0;i<N;i++)
        {
        vx[i]=vx[i]*3.0;            
        ...
        if (contador==9999||contador==14999||contador==17499||contador==22499)      //Multiplicar la velocidad en los tiempo del apartado 6
        {
        for(i=0;i<N;i++)
        {
        vx[i]=vx[i]*1.5;
        ...
        /*if (contador==59999||contador==89999||contador==119999||contador==29999||contador==149999)        //Multiplicar la velocidad en los tiempo del apartado 7
        {
        for(i=0;i<N;i++)
        {
        vx[i]=vx[i]*1.1;
        ...
        T=0;
           ...
           /* dx=x[6]-xi;               //Calcular desplazamiento respecto al inicial
            dy=y[6]-yi;
            r=sqrt(pow(dx,2)+pow(dy,2));
            if (abs(dx)<=(L/2.) && abs(dy)<=(L/2.))
            {
